...
// Extend CustomError to include axios response
interface AxiosCustomError extends CustomError {
  response?: {
    data?: {
      message?: string;
    };
...
export default function CreateTaskForm(props: {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    mode: "onChange", // Show errors immediately when they occur
    defaultValues: {
      title: "",
      description: "",
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    mutate(payload, {
      ...
      onError: (error: AxiosCustomError) => {
        ...
        // Check for specific assignment validation errors
        if (
          errorMessage &&
          (errorMessage.includes("Members cannot assign tasks to Owners") ||
            errorMessage.includes("Members cannot assign tasks to Admins") ||
            errorMessage.includes("Admins cannot assign tasks to Owners"))
        ) {
          // Clear any existing errors first
          form.clearErrors("assignedTo");

          // Set form error on the assignedTo field
          form.setError("assignedTo", {
            type: "manual",
            message: errorMessage,
          });

          // Return early to prevent toast from showing
          return;
        }
